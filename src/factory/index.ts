import { Address, beginCell, Cell, Contract, ContractProvider, Dictionary, SenderArguments, toNano } from '@ton/core';
import { storeSortedAssetsNested, storeCoinsNested, normalize } from '@torch-finance/core';
import { NextType } from '../common/types';
import {
  coinsMarshaller,
  getLpAccountProof,
  getMetaPoolProof,
...
import { Op } from '../common/opcodes';
import { Pool } from '../pool';
import { LpAccount } from '../lp-account';
...
import {
  ...
  RawWithdrawNext,
  RawWithdrawPayload,
  RawCancelDepositPayload,
} from './type';
import { Allocation, SignedRate, AssetType } from '@torch-finance/core';
import { ParsedDepositParams } from '@torch-finance/core';
import { Maybe } from '@ton/core/dist/utils/maybe';
import { ExactInParamsSchema, ParsedSwapParams } from '@torch-finance/core';
export class Factory implements Contract {
  ...
  private _createNextCell(
    ...
    if (currentOp === 'deposit' && next.$$type !== 'DepositNext' && next.$$type !== 'SwapNext') {
      throw new Error('Deposit operation can only have DepositNext or SwapNext');
    }

    if (currentOp === 'swap' && next.$$type !== 'SwapNext' && next.$$type !== 'WithdrawNext') {
  ...
  private _createDepositCell(src: RawDepositPayload) {
    ...
    if (!Array.isArray(src.depositAmounts)) {
      throw new Error('DepositAmounts should be an array in the internal function');
    }

    let assetsCell: Cell;
    const targetsCell = storeCoinsNested(src.depositAmounts.map((alloc) => alloc.amount));

    const nonZeros = src.depositAmounts.filter((alloc) => alloc.amount > 0);
    if (
      (nonZeros.length === 1 && !src.next) ||
      (nonZeros.length === 1 && src.next && (src.next!.$$type === 'SwapNext' || src.next?.metaAmount === 0n))
    ) {
      const { asset } = src.depositAmounts[0];
      assetsCell = beginCell().storeRef(asset.toCell()).endCell();
    } else {
      assetsCell = storeSortedAssetsNested(src.depositAmounts.map((alloc) => alloc.asset));
    }
      ...
      .storeMaybeRef(
        src.params
          ? beginCell()
              ...
              .storeMaybeRef(src.params.signedPrice ? this._createSignedPrice(src.params.signedPrice) : null)
  ...
  private _createSignedPrice(src: SignedRate): Cell {
    return beginCell()
      .storeBuffer(src.signature, 64)
      .storeRef(src.payload.toCell())
      .storeMaybeRef(src.nextSignedRate ? this._createSignedPrice(src.nextSignedRate) : null)
      .endCell();
  }
  ...
  private _createWithdrawNextCell(src: RawWithdrawNext): Cell {
    ...
    return beginCell().storeUint(NextType.WITHDRAW, 2).storeAddress(src.nextPool).storeMaybeRef(assetOut).endCell();
  }
  ...
  private _createSwapCell(src: RawSwapPayload) {
    ...
    return beginCell()
      ...
      .storeRef(src.assetOut.toCell())
      .storeUint(src.deadline ?? BigInt(Math.floor(Date.now() / 1000) + 10 * 60), 32) // default deadline is 10 minutes
      .storeMaybeRef(
        src.params
          ? beginCell()
              .storeCoins(src.params.minAmountOut ?? 0)
  ...
  private _createWithdrawCell(src: RawWithdrawPayload) {
    ...
    if (params?.minAmountOuts) {
      ...
      if (nonZeros.length > 1) {
        ...
        minAmountOuts = toNestedCoinCell(params.minAmountOuts.map((alloc) => alloc.amount));
      } else {
        // Get index of non-zero allocation
        const index = params.minAmountOuts.findIndex((alloc) => alloc.amount > 0);
    ...
    const assetOut = src.params?.removeOneAsset ? src.params.removeOneAsset.toCell() : null;
    const nextCell = src.next ? this._createNextCell(src.next, 'withdraw') : null;

    return beginCell()
      .storeUint(Op.Withdraw, 32)
      .storeAddress(recipient)
      .storeMaybeRef(params?.signedPrice ? this._createSignedPrice(params.signedPrice) : null)
  ...
  private _getDepositPayload = async (
    ...
    const allocs = Array.isArray(originDeposits) ? originDeposits : [originDeposits];

    // if depositAmounts is an array, expand it with zero allocations for other assets
    // ALERT: SHOULD CHECK ASSET IS SORTED
    const normalized = normalize(allocs, poolData.assets);
    payload.depositAmounts = normalized;

    const rejectTON = payload.params?.rejectPayload?.value ?? 0n;

    // Should use the original depositAmounts order for the senderArgs
    for (let i = 0; i < allocs.length; i++) {
      const { asset, amount } = allocs[i];
      const fulfillTON = i === allocs.length - 1 ? (payload.params?.fulfillPayload?.value ?? 0n) : 0n;
      const sendTON = fulfillTON > rejectTON ? fulfillTON : rejectTON;
      switch (asset.type) {
        case AssetType.TON: {
          const vaultAddr = await this.getAddress(provider, getVaultProof(asset));
          senderArgs.push({
            to: vaultAddr,
            value: amount + config.depositGas + sendTON,
            body: beginCell()
              .storeUint(Op.Deposit, 32)
        ...
        case AssetType.JETTON: {
          ...
          const jetton = provider.open(JettonMaster.create(asset.jettonMaster as Address));
          ...
          senderArgs.push({
            to: jettonWallet,
            value: config.jettonGas + config.depositGas + sendTON,
            body: beginCell()
              ...
              .storeCoins(config.depositGas + sendTON)
  ...
  getDepositPayload = async (
    ...
    if (secondPool && params.nextDeposit) {
      ...
      const metaAsset = secondPool.assets.find((asset) => !asset.equals(Asset.jetton(firstPool.lpAsset.jettonMaster!)));
  ...
  private _getSwapPayload = async (
    ...
    const vaultAddr = await this.getAddress(provider, getVaultProof(payload.assetIn));

    const fulfillTON = payload.params?.fulfillPayload?.value ?? 0n;
    const rejectTON = payload.params?.rejectPayload?.value ?? 0n;
    const sendTON = fulfillTON > rejectTON ? fulfillTON : rejectTON;
    switch (payload.assetIn.type) {
      case AssetType.TON:
        return {
          to: vaultAddr,
          value: config.swapGas + payload.amountIn + sendTON,
          body: beginCell()
            .storeUint(Op.Swap, 32)
            .storeUint(payload.queryId, 64)
      ...
      case AssetType.JETTON: {
        const jetton = provider.open(JettonMaster.create(payload.assetIn.jettonMaster as Address));
        const jettonWallet = await jetton.getWalletAddress(sender);
        const customPayload = null;
        const forwardPayload = this._createSwapCell(payload);
        return {
          to: jettonWallet,
          value: config.jettonGas * 2n + config.swapGas + sendTON,
          body: beginCell()
            .storeUint(Op.JettonTransfer, 32)
            .storeUint(payload.queryId, 64)
            .storeCoins(payload.amountIn)
            ...
            .storeAddress(sender)
            .storeMaybeRef(customPayload)
            .storeCoins(config.swapGas + sendTON)
            .storeMaybeRef(forwardPayload)
            .endCell(),
        };
      }
  ...
  private _buildSwapNextFromHops(hops: Hop[]): RawSwapNext | RawWithdrawNext | RawDepositNext | null {
    if (hops.length === 0) {
      return null;
    }
    ...
    if (firstRoute.action === 'swap') {
      return {
        ...
        next: this._buildSwapNextFromHops(restRoutes) as RawSwapNext | RawWithdrawNext,
      };
      ...
      return {
        ...
        nextPool: firstRoute.pool.address,
        metaAmount: 0n,
        metaAsset: firstRoute.pool.assets.find((asset) => !asset.equals(Asset.jetton(stablePool.address)))!,
      };
    }
    return null;
  }
  ...
  getSwapPayload = async (
    ...
    const { queryId, assetIn, assetOut, amountIn } = ExactInParamsSchema.parse(params);
    ...
    if (firstRoute.action === 'swap') {
      ...
      const sendArgs = await this._getDepositPayload(
        ...
        sender,
        {
          ...
          next: this._buildSwapNextFromHops(restRoutes) as RawDepositNext | RawSwapNext,
        },
        poolData,
      );
      return sendArgs[0];
  ...
  private _getWithdrawPayload = async (
    ...
    if (payload.params && Array.isArray(payload.params.minAmountOuts)) {
      ...
      if (payload.next?.nextPool) {
        const nextPool = provider.open(Pool.createFromAddress(payload.next.nextPool));
        const nextPoolAssets = await nextPool.getAssets();
        poolToUse = nextPoolAssets; // update to use next pool's assets
      }
      // ALERT: SHOULD CHECK ASSETS ARRAY `poolToUse` IS SORTED
      const normalized = normalize(payload.params.minAmountOuts, poolToUse);
    ...
    const jetton = provider.open(JettonMaster.create(lpAsset.jettonMaster as Address));
    const jettonWallet = await jetton.getWalletAddress(sender);
    const customPayload = null;
  ...
  async getWithdrawPayload(
    ...
    const pools: [Address, Address?] = params.nextWithdraw ? [params.pool, params.nextWithdraw.pool] : [params.pool];
    ...
    if (params.mode === 'Single') {
      ...
      } else if (params.nextWithdraw && params.nextWithdraw.mode === 'Balanced') {
  ...
  async getCancelDepositPayload(
    provider: ContractProvider,
    params: RawCancelDepositPayload,
    config: {
      jettonGas: bigint;
      cancelGas: bigint;
    } = { jettonGas: toNano('0.05'), cancelGas: toNano('0.1') },
  ): Promise<SenderArguments> {
    const lpAccount = provider.open(LpAccount.createFromAddress(params.lpAccount));
    const lpAccountData = await lpAccount.getLpAccountData();

    // calculate the gas based on the asset transfers
    const nonZeroBalances = lpAccountData.balances.filter((balance) => balance.amount > 0n);
    const baseGas =
      config.cancelGas +
      BigInt(nonZeroBalances.length) * config.jettonGas +
      (lpAccountData.metaAmount ? config.jettonGas : 0n);

    return {
      to: params.lpAccount,
      value: baseGas,
      body: beginCell().storeUint(Op.CancelDeposit, 32).storeUint(params.queryId, 64).endCell(),
    };
  }
  ...
  async getStablePool(provider: ContractProvider, assets: Asset[]): Promise<Pool> {
    const sorted = assets.sort((a, b) => a.compare(b));
    const proof = getStablePoolProof(storeSortedAssetsNested(sorted));
    return this.getPool(provider, proof);
  }

  async getMetaPool(provider: ContractProvider, assets: Asset[]): Promise<Pool> {
    const sorted = assets.sort((a, b) => a.compare(b));
    const proof = getMetaPoolProof(storeSortedAssetsNested(sorted));
    return this.getPool(provider, proof);
  }
